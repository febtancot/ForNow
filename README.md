...
没有可用刘海的屏幕环境会使用默认屏幕菜单栏下方中央作为入口。已选择的显示器全部断开时，胶囊会临时显示在默认屏幕上，重新接入后自动恢复。关闭全部胶囊显示后，带刘海屏仍保留刘海下方的透明触发区，可点击或接收拖入；无刘海屏不会设置不可见热区。菜单栏图标和全局快捷键仍可在可用屏幕临时打开面板，收起后不会显示胶囊；菜单栏图标也继续提供设置和检查更新入口。
...
当前 `ForNowKitTests` 包含 139 个单元测试，不依赖应用宿主。单元测试覆盖数据模型、
暂存备注与持久化、存储、去重、历史 TXT 文字项归并、回收站、剪贴板与拖放归类、
外部文件接收契约、快捷键模型、音频播放状态、屏幕选择三态与旧偏好迁移、面板几何和 DayDrop
联动契约等纯逻辑。
