...
没有可用刘海的屏幕环境会使用屏幕顶部中央区域作为入口。菜单栏图标始终提供打开面板、设置和检查更新的备用入口。
...
当前 `ForNowKitTests` 包含 81 个单元测试，不依赖应用宿主。单元测试覆盖数据模型、存储、去重、回收站、剪贴板归类、快捷键模型、音频播放状态和面板几何等纯逻辑。

命令行构建和单元测试不能证明刘海点击、边缘拖动、系统拖放、麦克风录音、实际音频输出或 VoiceOver 交互已经通过。发布前仍需在真实 Mac 上完成这些交互检查。
