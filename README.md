...
没有可用刘海的屏幕环境会使用默认屏幕菜单栏下方中央作为入口。已选择的显示器全部断开时，胶囊会临时显示在默认屏幕上，重新接入后自动恢复。关闭全部胶囊后，菜单栏图标和全局快捷键仍可在可用屏幕临时打开面板，收起后不会留下胶囊；菜单栏图标也继续提供设置和检查更新入口。
...
- 声音反馈和动画效果；
- 面板宽度及恢复默认宽度；
- 小药丸吸附的屏幕，可选择一块、多块或全部关闭；首次使用及「恢复默认显示」时开启一块默认屏幕。应用常驻监听显示器热插拔、排列和模式变化，待屏幕列表稳定后自动更新，也可手动点击「刷新显示器列表」；
- 显示或隐藏面板的全局快捷键；
- 手动检查更新和查看更新记录。
...
当前 `ForNowKitTests` 包含 136 个单元测试，不依赖应用宿主。单元测试覆盖数据模型、
