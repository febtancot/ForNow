...
![Version](https://img.shields.io/badge/version-0.7.9-2563EB)
...
- 点击面板底部的输入区，输入文字或 HTTP(S) URL 后按回车。内容超过当前输入区宽度时会自动换行，最多显示 8 行，继续输入可在输入区内滚动。
...
- 右键任意暂存项目并选择「添加备注」，可记录用途、来源或后续动作；备注会显示在项目名称下方。再次右键可编辑，清空内容后保存可移除备注。
...
- 首次使用时，小药丸默认吸附在 MacBook 刘海下方；有外接显示器时，可在设置中选择任意一块、多块或关闭全部屏幕上的胶囊。外接屏入口位于菜单栏下方中央。

没有可用刘海的屏幕环境会使用默认屏幕菜单栏下方中央作为入口。已选择的显示器全部断开时，胶囊会临时显示在默认屏幕上，重新接入后自动恢复。关闭全部胶囊显示后，带刘海屏仍保留刘海下方的透明触发区，可点击或接收拖入；无刘海屏不会设置不可见热区。全局快捷键会读取触发瞬间的鼠标位置，在鼠标所在屏幕临时打开面板，即使该屏幕未启用胶囊；收起后会移除临时窗口。若该屏幕正被全屏策略屏蔽，则不会改到另一块屏幕打开。菜单栏图标继续按默认落屏策略打开面板，并提供设置和检查更新入口。
...
| 操作 | 快捷键 |
| --- | --- |
| 显示或隐藏面板 | `⌃⌥Space`，可在设置中修改；打开时跟随鼠标所在屏幕 |
| 粘贴并暂存 | `⌘V` |
| 复制所选项目 | `⌘C` |
| 全选 | `⌘A` |
| 移到回收站 | `Delete` |
...
- 登录时启动；
- 是否在全屏应用中显示胶囊（默认关闭，避免覆盖全屏视频）；
- 声音反馈和动画效果；
- 面板宽度及恢复默认宽度；
- 小药丸吸附的屏幕，可选择一块、多块或关闭全部胶囊显示；全部关闭时，带刘海屏仍保留刘海下方的点击和拖入入口，无刘海屏依赖菜单栏图标或全局快捷键。首次使用及「恢复默认显示」时开启一块默认屏幕。应用常驻监听显示器热插拔、排列和模式变化，待屏幕列表稳定后自动更新，也可手动点击「刷新显示器列表」；
- 显示或隐藏面板的全局快捷键；快捷键打开时跟随鼠标所在屏幕；
- 手动检查更新和查看更新记录。
...
当前 `ForNowKitTests` 包含 146 个单元测试，不依赖应用宿主。单元测试覆盖数据模型、
暂存备注与持久化、存储、去重、历史 TXT 文字项归并、回收站、剪贴板与拖放归类、
外部文件接收契约、快捷键模型、音频播放状态、屏幕选择三态与旧偏好迁移、鼠标所在屏幕解析、快速输入换行与高度测量、面板几何和 DayDrop
联动契约等纯逻辑。
