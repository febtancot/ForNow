...
- 当前 `main` 已允许关闭所有视觉胶囊，同时在刘海屏保留透明点击和拖入触发区。三态设置、旧偏好迁移、历史断开 ID、热插拔保持全关、隐藏触发屏筛选和刘海最小热区回退已有纯逻辑测试；完整 139 项单元测试通过。本机 `/Applications/ForNow.app` 当前仍是提交 `acf718b` 的同版本号 0.7.8（12）本地构建，尚未包含本次刘海触发修正。实际刘海点击/拖入、全关无残留视觉、外接屏无隐形拦截、菜单栏/快捷键、全屏、热插拔、跨重启和 VoiceOver 仍需本机交互验收；尚未重新安装、打包或发布新版本。
