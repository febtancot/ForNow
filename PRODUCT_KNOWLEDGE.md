...
- 当前 `main` 已实现尚未发布的快捷键鼠标跟屏：快捷键展开时严格使用鼠标所在屏幕，未启用胶囊的屏幕使用临时窗口，收起后清理；目标屏被全屏策略屏蔽时不跨屏回退。完整 142 项单元测试通过；实际多屏落点仍需在本地安装构建后验收。
...
- **2026-09-04 · 快捷键跟随鼠标所在屏幕**：全局快捷键注册回调在触发瞬间读取 `NSEvent.mouseLocation`，按 `NSScreen.frame` 匹配目标显示器，并通过快捷键专用入口展开；菜单栏按钮继续使用原有默认落屏策略。已选屏和隐藏刘海入口直接复用现有窗口；鼠标位于未启用胶囊的已连接屏幕时创建临时窗口，收起后删除。临时窗口在显示器刷新期间只要仍连接便继续保留，若随后成为持久选择则自动转为常驻窗口。目标屏断开或被全屏策略屏蔽时，本次操作不打开，也不静默跳到其他屏幕。新增 3 项坐标边界、镜像顺序和严格目标选择测试，完整 142 项单元测试通过；本地安装与双屏交互验收另行记录。
