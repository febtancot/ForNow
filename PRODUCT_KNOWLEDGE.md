...
- 当前 `main` 已实现尚未发布的快速输入自动换行修复：输入区使用实际布局宽度驱动 TextKit 测高，修复可调面板和清空按钮显隐造成的临界行裁切，并补上连续输入节流的尾沿测量。完整 146 项单元测试通过；本地安装与真实输入区滚动验收尚未完成。
...
- **2026-09-04 · 快速输入按实际宽度自动换行**：修复垂直 TextField 已折行、但外层输入区仍按固定 324pt 测高而裁掉临界新行的问题。`QuickEntryField` 现在读取 TextField 最终布局宽度，并预留 4pt 文本/滚动视口边缘误差后交给 `DraftModel`；面板在 320–720pt 范围缩放、清空按钮出现或消失时都会重新测高。`DraftTextMetrics` 继续限制 4,000 字符前缀和 8 行排版，不再先执行 O(全文) 的 `String.count`；无空格英文长串按可用宽度折行。原每秒一次且可能吞掉最终跨行状态的节流，改为 100ms 限频加尾沿重测，兼顾跟手与大文本性能。新增 4 项无空格折行、宽度重排、无效宽度和尾沿时序回归测试，完整 146 项单元测试通过；本地安装和真实 1–8 行增长、8 行后滚动仍待验收。
