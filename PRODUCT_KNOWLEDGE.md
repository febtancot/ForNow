...
| 多屏吸附 | ✅ | 首次使用默认在一块屏幕显示胶囊；设置列出当前显示器（含左右/上下位置），可单选、多选或全部关闭，并保留「刷新显示器列表」。自动默认、指定屏幕和全部关闭是三个独立持久化状态；旧版空选择继续迁移为自动默认，避免升级后意外全关。应用常驻监听 Core Graphics 显示器重配置；忽略配置开始事件，合并同轮多屏回调，并在 `NSScreen` UUID 列表连续稳定后自动同步设置列表与小药丸窗口。每块已选屏幕各有一个收起态小药丸，点击哪块就只在哪块展开；外接/无刘海屏在菜单栏下沿保留 2pt 间距。显示器 UUID 跨重启保存，断开时临时回退默认屏，重连自动恢复；全部关闭后热插拔不会重新显示胶囊，菜单栏图标或全局快捷键仍可在可用默认屏临时展开面板，收起后不留下胶囊 |
...
- **多屏窗口模型**：`DisplayAttachmentSelection` 使用自动默认、指定屏幕和全部关闭三态；首次使用及「恢复默认显示」优先选择带刘海的屏幕，其次主屏。`NotchController` 按设置为每块目标屏幕维护独立 `NotchWindow`，共享同一份暂存、草稿、录音和播放状态。所有目标屏幕可同时显示收起态小药丸，但任一时刻只允许一块屏幕展开；点击另一块屏幕会把面板切换过去。全部关闭时不保留收起窗口；菜单栏、全局快捷键或外部文件导入仍可在未被全屏内容覆盖的默认屏按需创建临时面板，面板收起后立即移除。无刘海屏使用 `visibleFrame.maxY` 作为吸附线，胶囊在窗口内改为顶部对齐并保留 2pt 间距；刘海屏仍在刘海下方按原逻辑底部对齐。屏幕参数变化后立即重建或重定位窗口。
...
- `ForNowKitTests`：136 个单测，无需 app host。
- 关键文件：`NotchController`（多屏常驻与临时窗口/开合/宽度调整/拖入/粘贴/选择/反馈/录音与播放协调）、`FullScreenVisibilityMonitor`（后台 WindowServer 轮询与逐屏显隐）、`FullScreenWindowDetector`（全屏覆盖纯逻辑）、`DropFileURLLoader`/`DropFileURLDecoder`（拖放文件 URL 优先读取与原始表示解码）、`DisplayIdentity`（ColorSync 显示器 UUID 与 Quartz bounds）、`DisplayAttachmentSelection`（自动默认/指定屏幕/全部关闭、断开回退和切换纯逻辑）、`RecordingController`（AVAudioRecorder 录音状态机）、`AudioPlaybackController`（AVAudioPlayer 单实例播放/进度/定位）、`ContentHasher`（文件内容 SHA-256）、`DraftModel`（输入条独立状态）、`DraftTextMetrics`（截断高度测量）、`NotchMetrics`（含菜单栏下吸附线的几何）、`AppSettings`（持久化面板宽度与屏幕选择）、`StashStore`（暂存与回收站仓库）、`DiskFileStorage`/`JSONMetadataStore`/`JSONTrashMetadataStore`（持久化）、`PasteboardImporter`（归类）、`StatusItemController`（菜单栏）、`UpdaterModel`（Sparkle 更新桥接，KVO → SwiftUI）。
...
- 当前 `main` 已允许在设置中关闭所有显示器上的收起态胶囊，同时保持首次使用默认开启一块屏幕。三态设置、旧偏好迁移、历史断开 ID、热插拔保持全关和临时面板落屏已有纯逻辑测试；完整 136 项单元测试通过。本机 `/Applications/ForNow.app` 已替换为同版本号 0.7.8（12）的 Universal 本地 Release 构建并从明确路径启动。设置实际点击、全关后的胶囊消失、菜单栏/快捷键临时展开与跨重启行为仍需本机交互验收；本地构建未公证，尚未打包或发布新版本。
