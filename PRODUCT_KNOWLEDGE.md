...
| 快速录入 | ✅ | 面板底部输入条，**点击聚焦**后打字，回车入库置顶并收起；纯文字建文字项、http(s) 地址自动建链接项；内容超过输入区实际宽度时自动折行，无空格长串也能换行；上限 8 行，超出后内部滚动 |
...
| 全局快捷键 | ✅ | 默认 ⌃⌥Space（Carbon `RegisterEventHotKey`）；触发时读取 `NSEvent.mouseLocation`，严格在鼠标所在屏幕打开面板，即使该屏幕未启用胶囊 |
| 反馈 | ✅ | toast + 声音；深色模式、VoiceOver 标签 |
| 多屏吸附 | ✅ | 首次使用默认在一块屏幕显示胶囊；设置列出当前显示器（含左右/上下位置），可单选、多选或关闭全部胶囊显示，并保留「刷新显示器列表」。自动默认、指定屏幕和全部关闭是三个独立持久化状态；旧版空选择继续迁移为自动默认，避免升级后意外全关。应用常驻监听 Core Graphics 显示器重配置；忽略配置开始事件，合并同轮多屏回调，并在 `NSScreen` UUID 列表连续稳定后自动同步设置列表与窗口。每块已选屏幕各有一个收起态小药丸，点击哪块就只在哪块展开；外接/无刘海屏在菜单栏下沿保留 2pt 间距。显示器 UUID 跨重启保存，断开时临时回退默认屏，重连自动恢复。全部关闭后不绘制任何胶囊，但带刘海屏仍保留无阴影的透明点击和拖入热区；无刘海屏不创建不可见热区。热插拔不会重新显示胶囊；菜单栏按原有默认策略落屏，全局快捷键则严格跟随鼠标所在屏幕 |
...
- **刘海命中区**：刘海缺口本身无可点击像素，真正命中的是缺口正下方一条覆盖刘海宽度、下延 26pt 的热区；透明命中层用 `Color.white.opacity(0.001)`（`Color.clear` 只响应悬停、不响应点击）。屏幕重配置期间 auxiliary 区域尚未就绪但 `safeAreaInsets.top > 0` 时，先保留 160pt 宽的中央最小热区，避免入口短暂消失。
...
- **多屏窗口模型**：`DisplayAttachmentSelection` 使用自动默认、指定屏幕和全部关闭三态；首次使用及「恢复默认显示」优先选择带刘海的屏幕，其次主屏。`NotchController` 分别维护可见胶囊屏、隐藏刘海触发屏和临时面板屏，共享同一份暂存、草稿、录音和播放状态。所有已选屏幕可同时显示收起态小药丸，但任一时刻只允许一块屏幕展开；点击另一块屏幕会把面板切换过去。全部关闭时，带刘海屏保留覆盖刘海及其下方 26pt 的透明窗口，只渲染命中层，不实例化 mic、托盘或 DayDrop 胶囊内容，并关闭窗口阴影；点击或拖入仍可展开，收起后恢复透明热区。无刘海屏不保留不可见窗口；菜单栏和外部文件导入继续使用原有默认落屏策略。全局快捷键单独读取 `NSEvent.mouseLocation`，以 AppKit 全局坐标匹配 `NSScreen.frame`；鼠标所在屏幕即使未启用胶囊，也会按需创建临时面板，并在收起后立即移除。若目标屏幕断开或被全屏策略屏蔽，本次快捷键不打开面板，也不回退到其他屏幕。无刘海可见胶囊使用 `visibleFrame.maxY` 作为吸附线并保留 2pt 间距；刘海屏可见胶囊仍在刘海下方按原逻辑底部对齐。屏幕参数变化后立即重建或重定位窗口。
...
- **快速录入（输入条，面板底部）**：面板打开**不**自动聚焦（保证 ⌘V 走传统粘贴入库），点击输入条才开始打字；回车（或点击「收起」）提交——空草稿只收起、非空建项置顶（http(s) 地址建链接项）并收起；Esc 有草稿先清空、无草稿收起；输入条聚焦期间 `⌘V`/`⌘A`/Delete 由文本框原生处理（不走列表快捷键）。输入条按 TextField 最终布局宽度测量内容；面板宽度变化或清空按钮显隐后会重新测量，文字、URL、哈希等无空格长串超过可用宽度时也会折行。最多展示 8 行，超出后**定高滚动、显示滚动条**。**状态隔离与高度测量**：草稿/聚焦状态在独立 `DraftModel`（仅输入条观察），打字/粘贴只重绘输入条；高度测量用 `DraftTextMetrics`，直接截取前 4,000 个字符并最多排版 8 行，不先扫描全文。连续输入与面板拖宽最多每 100ms 测量一次，并保留尾沿任务，停手后一定使用最终文本与宽度重测。窗口高度由测量后的 `draftDidChange` 事件驱动 `NotchWindow.contentHeight` 直接调整，避免大段文字触发逐帧全量排版。
...
- `ForNowKitTests`：146 个单测，无需 app host。
- 关键文件：`NotchController`（多屏可见胶囊、隐藏刘海触发区与临时窗口/开合/宽度调整/拖入/粘贴/选择/反馈/录音与播放协调）、`FullScreenVisibilityMonitor`（后台 WindowServer 轮询与逐屏显隐）、`FullScreenWindowDetector`（全屏覆盖纯逻辑）、`DropFileURLLoader`/`DropFileURLDecoder`（拖放文件 URL 优先读取与原始表示解码）、`DisplayIdentity`（ColorSync 显示器 UUID 与 Quartz bounds）、`DisplayAttachmentSelection`（自动默认/指定屏幕/全部关闭、隐藏刘海触发区、断开回退和切换纯逻辑）、`RecordingController`（AVAudioRecorder 录音状态机）、`AudioPlaybackController`（AVAudioPlayer 单实例播放/进度/定位）、`ContentHasher`（文件内容 SHA-256）、`DraftModel`（输入条独立状态）、`DraftTextMetrics`（截断高度测量）、`NotchMetrics`（含菜单栏下吸附线的几何）、`AppSettings`（持久化面板宽度与屏幕选择）、`StashStore`（暂存与回收站仓库）、`DiskFileStorage`/`JSONMetadataStore`/`JSONTrashMetadataStore`（持久化）、`PasteboardImporter`（归类）、`StatusItemController`（菜单栏）、`UpdaterModel`（Sparkle 更新桥接，KVO → SwiftUI）。
...
- 0.7.9（build 13）已发布：设置可关闭所有视觉胶囊，同时在刘海屏保留透明点击入口；首次使用、旧偏好迁移与「恢复默认显示」仍默认开启一块屏幕。139 项单元测试通过，Universal DMG 完成 Developer ID 签名、Apple 公证与票据装订；Sparkle appcast、build 12 增量包、独立发布说明和 Cloudflare Pages 生产站已更新。本机 `/Applications/ForNow.app` 已通过 Sparkle 安装正式 0.7.9，并从设置页确认线上检查结果为当前最新版本。隐藏态 Finder 拖入、全屏、热插拔、像素级无残留和 VoiceOver 仍需人工验收。
- 0.8.0（build 14）已发布：全局快捷键在面板关闭时读取鼠标位置并严格在目标屏幕打开；未启用胶囊的屏幕使用收起即清理的临时面板。快速输入按实际可见宽度折行，无空格长串同样适用，最多展示 8 行后内部滚动。146 项单元测试通过，Universal DMG 已完成 Developer ID 签名、Apple 公证与票据装订；Sparkle appcast、build 13 增量包、独立发布说明和 Cloudflare Pages 生产站已更新。本机已从公证 DMG 可恢复安装正式 0.8.0；生产 Sparkle 的 UI 下载与安装因 Mac 锁屏未验收。物理快捷键双屏操作、输入法组合态、物理键盘连续输入手感和输入区滚轮操作仍需人工验收。
...
- **2026-09-03 · 允许关闭全部显示器胶囊**：设置中的最后一块显示器现在可关闭。`DisplayAttachmentSelection` 改为自动默认、指定屏幕、全部关闭三态，并新增 mode key；新安装、旧版缺失 key 与旧空数组仍为自动默认，明确全关可跨重启保存。关闭最后一块当前有效屏幕时直接进入全关，历史断开显示器 ID 不会触发默认屏回退；热插拔也不会恢复胶囊。「恢复默认显示」重新开启默认屏。全关只隐藏收起态胶囊，菜单栏、全局快捷键及外部文件导入仍可在可用且未被全屏内容覆盖的默认屏临时展开面板，收起后移除窗口。新增 19 项选择、迁移和临时落屏测试，完整 136 项单元测试通过；本机设置交互、真实窗口显隐与跨重启仍待人工验收，未打包或发布。
- **2026-09-03 · 全关胶囊功能本地安装**：从提交 `acf718b` 执行 clean Universal Release 构建，以 `Developer ID Application: Xueliu Shen (8NF4K823FV)` 签名后，通过独立 pending 副本完成深度签名、版本和哈希预检，再可恢复地替换 `/Applications/ForNow.app`。已安装 App 仍显示 0.7.8（12），主程序 SHA-256 为 `cda6689367f37434ebcf17c7a337085b5d723a941acfc4e4340a7fe96d8c3da7`，与最终构建一致；进程从明确安装路径启动并持续存活。原已公证正式版保存在 `/Users/liuyuhang/Library/Application Support/ForNow Installer Backups/ForNow-before-all-capsules-off-20260903-214250.app`，主程序 SHA-256 仍为 `6919275980c7ffcedaff06b6a2daa6ac9f375c47c593f72a5f24a6610d948be0`，Gatekeeper 仍识别为 `Notarized Developer ID`。新安装副本仅为 `Unnotarized Developer ID` 本地构建；未生成 DMG、未公证、未发布网站或 Sparkle。Application Support 用户数据未清理；设置点击、多屏真实显隐、快捷键临时面板、跨重启和 VoiceOver 仍待人工验收。
- **2026-09-03 · 全关后保留刘海触发区**：修复 `.disabled` 同时删除视觉胶囊窗口和刘海透明命中层的回归。可见胶囊 ID 与持久交互窗口 ID 现分开解析：全关时只在实际刘海屏保留无阴影透明窗口，`ClosedPillView` 不渲染 `ClosedCapsuleBar`，但继续保留点击、首次点击和文件拖入链路；无刘海外接屏不创建不可见热区。菜单栏、快捷键和外部导入优先复用未被全屏覆盖的隐藏刘海窗口；无刘海环境继续使用收起即移除的临时面板。若屏幕重配置时 auxiliary 区域暂未就绪但安全区已确认刘海，则保留 160pt 宽最小热区。新增 3 项隐藏触发屏与几何回退测试，完整 139 项单元测试通过；真实刘海命中、拖入、阴影/胶囊无残留、全屏与多屏热插拔仍待本机交互验收，尚未重新安装或发布。
- **2026-09-03 · 刘海触发修正本地安装与点击验收**：从提交 `abdf1d7` clean 构建 Universal Release，并以 `Developer ID Application: Xueliu Shen (8NF4K823FV)` 签名；通过独立 pending 副本预检后可恢复替换 `/Applications/ForNow.app`。已安装版本仍为 0.7.8（12），主程序 SHA-256 `75a136a1a777d3747bea4bdbccb5995404706516a5c039b9fc89ffe20fa94912` 与构建产物一致，进程从明确安装路径启动。上一份本地构建保存在 `/Users/liuyuhang/Library/Application Support/ForNow Installer Backups/ForNow-before-notch-trigger-fix-20260903-221501.app`，主程序 SHA-256 为 `cda6689367f37434ebcf17c7a337085b5d723a941acfc4e4340a7fe96d8c3da7`；更早的已公证正式备份继续保留。实机将内建屏与 LG 的胶囊开关全部关闭后，设置持久化为 `disabled`，辅助功能树不再出现收起态 mic、托盘或 DayDrop 控件；WindowServer 仅显示内建屏顶部 `220×64`、layer 25 的 ForNow 透明窗口，LG 无常驻 ForNow 窗口。点击刘海下方命中带可展开 9 项面板，收起后再次点击仍成功；退出并从 `/Applications/ForNow.app` 重启后，`disabled` 保持且刘海点击再次成功。最终已按 Esc 收起并保持全部胶囊关闭。未执行 Finder 拖入、全屏、热插拔、屏幕像素或 VoiceOver 验收；新安装仍是 `Unnotarized Developer ID` 本地构建，未生成 DMG、未公证、未发布网站或 Sparkle，Application Support 用户数据未清理。
- **2026-09-03 · v0.7.9 全关胶囊版本发布**：版本升至 0.7.9（build 13），发布提交为 `32280fd`。独立 DerivedData 的 139 项测试全部通过；`dist/ForNow-0.7.9.dmg` 为 4,545,260 bytes，SHA-256 `f8f4f59c196faec5e4c04c8bd3cde2dcf7e73750b791866bf9d07c07a2c7f3cd`，完成 Developer ID 签名、Apple 公证 `Accepted`（提交号 `a88b9aa5-ab33-445f-9563-273ea5fcf7ff`，公证日志 0 个问题）与票据装订。DMG 及只读挂载后的 App 均由 Gatekeeper 识别为 `Notarized Developer ID`；包内 App 为 0.7.9（13）、Universal `x86_64 arm64`，无 `get-task-allow`，主程序 SHA-256 `eb36c8212945b0403a06a72351fac858e8803455592d3e7e78137443ad1ff593`。Sparkle appcast SHA-256 为 `a978a2d3436774e62d79bd2853925306d20ef7bec529824c7ddf4bcf22627365`，首项为 build 13，并生成从 build 12/11/10/9/8 升级的 5 个已签名 delta；相邻 `ForNow13-12.delta` 为 272,398 bytes，SHA-256 `f0ca84e735ebedd27763a8e263499ee21c499117f496f840687cfde8a9405405`。实际把该 delta 应用于正式 0.7.8 后，重建 App 通过深度验签，主程序哈希和 CDHash 均与完整 0.7.9 包一致。Cloudflare Pages 生产部署 `ce852e8c-8a19-4d12-9b6b-134e8bae16be` 已上线；首页、独立发布说明、appcast、两份 DMG 和 5 个 delta 均完成线上状态、缓存头及哈希回读，本地与线上 DMG/feed 一致，不存在的版本返回 404。桌面 1440 px 与移动端 390 px 浏览器检查无横向溢出或破图，新版下载入口和更新卡片正确。生成 appcast 时历史文件已存在于 `old_updates`，出现 `Code=516` 警告；旧文件无缺失或变化，新 feed、签名、delta 与线上哈希完整，因此确认未影响发布。发布后 `/Applications/ForNow.app` 的内容变更时间、Sparkle Installation/PersistentDownloads 缓存和进程重启时间一致，安装结果为正式 0.7.9（13），主程序与 DMG 包内 App 一致且 Gatekeeper 通过；设置「更新」页显示 0.7.9（13），再次检查确认已是最新版本。已从正式 0.7.8 DMG 提取并保留可恢复备份 `/Users/liuyuhang/Library/Application Support/ForNow Installer Backups/ForNow-formal-0.7.8-before-0.7.9-20260903.app`。隐藏态 Finder 拖入、全屏、热插拔、像素级视觉和 VoiceOver 未随正式版重复验收；网站已发布，但源码提交尚未推送。
- **2026-09-04 · 快捷键跟随鼠标所在屏幕**：全局快捷键注册回调在触发瞬间读取 `NSEvent.mouseLocation`，按 `NSScreen.frame` 匹配目标显示器，并通过快捷键专用入口展开；菜单栏按钮继续使用原有默认落屏策略。已选屏和隐藏刘海入口直接复用现有窗口；鼠标位于未启用胶囊的已连接屏幕时创建临时窗口，收起后删除。临时窗口在显示器刷新期间只要仍连接便继续保留，若随后成为持久选择则自动转为常驻窗口。目标屏断开或被全屏策略屏蔽时，本次操作不打开，也不静默跳到其他屏幕。新增 3 项坐标边界、镜像顺序和严格目标选择测试，完整 142 项单元测试通过；本地安装见下一条，双屏交互验收尚未完成。
- **2026-09-04 · 快捷键跟屏功能本地安装**：从提交 `ec8faf6` 使用独立、全新的 DerivedData 构建 Universal Release，并以 `Developer ID Application: Xueliu Shen (8NF4K823FV)` 深度签名。已安装 `/Applications/ForNow.app` 保持 0.7.9（13），主程序 SHA-256 为 `1c548004d20599c46888f83eafab0a3ba086b643059ead50af6af6a85bc71b72`，与独立构建产物一致；深度验签通过，进程从明确安装路径启动。原已公证正式 0.7.9 保存在 `/Users/liuyuhang/Library/Application Support/ForNow Installer Backups/ForNow-formal-0.7.9-before-mouse-display-20260904-080700.app`，主程序 SHA-256 为 `eb36c8212945b0403a06a72351fac858e8803455592d3e7e78137443ad1ff593`，Gatekeeper 仍识别为 `Notarized Developer ID`。新安装副本是 `Unnotarized Developer ID` 本地测试构建；未覆盖正式 0.7.9 DMG，未更新 appcast、网站或 Sparkle。安装后两块屏幕的收起窗口与既有显示器选择均保留，实际点击外接屏胶囊可正常展开并收起面板。自动化触发自定义 `⇧⌥Space` 时，首次重试遇到锁屏；恢复后合成按键仍未到达 Carbon 全局热键处理器。因此没有把源码、构建、点击路径或 WindowServer 静态状态当作快捷键双屏交互通过；鼠标跟屏、未选屏临时窗口、全关模式和全屏 no-op 仍待人工按键验收。
- **2026-09-04 · 快速输入按实际宽度自动换行**：修复垂直 TextField 已折行、但外层输入区仍按固定 324pt 测高而裁掉临界新行的问题。`QuickEntryField` 现在读取 TextField 最终布局宽度，并预留 4pt 文本/滚动视口边缘误差后交给 `DraftModel`；面板在 320–720pt 范围缩放、清空按钮出现或消失时都会重新测高。1–7 行隐藏垂直 scroller，避免其保留槽位后再次缩窄实际视口；8 行封顶后才显示滚动条。`DraftTextMetrics` 继续限制 4,000 字符前缀和 8 行排版，不再先执行 O(全文) 的 `String.count`；无空格英文长串按可用宽度折行。原每秒一次且可能吞掉最终跨行状态的节流，改为 100ms 限频加尾沿重测，兼顾跟手与大文本性能。新增 4 项无空格折行、宽度重排、无效宽度和尾沿时序回归测试，完整 146 项单元测试通过；本地安装与交互结果见下一条。
- **2026-09-04 · 快速输入换行本地安装与交互验收**：从提交 `6ef7d50` 使用独立、全新的 DerivedData 构建 Universal Release，并以 `Developer ID Application: Xueliu Shen (8NF4K823FV)` 深度签名。已安装 `/Applications/ForNow.app` 保持 0.7.9（13），主程序 SHA-256 为 `e43a20ca7decf3a9879f64cdb3c34b7c047553e41f36bf393946a53c3304830a`，与独立构建一致；签名验证通过，Gatekeeper 状态为 `Unnotarized Developer ID`。上一份快捷键跟屏本地测试版保存在 `/Users/liuyuhang/Library/Application Support/ForNow Installer Backups/ForNow-local-mouse-display-before-quick-wrap-20260904-083224.app`，主程序 SHA-256 为 `1c548004d20599c46888f83eafab0a3ba086b643059ead50af6af6a85bc71b72`；已公证正式 0.7.9 备份继续保留。实际 App 中，421pt 面板输入 44 个连续 `W` 后，窗口从 470pt 增至 487pt，截图确认两行完整可见且无横向溢出；输入 320 个连续 `W` 后高度封顶 589pt，辅助功能树和截图均显示输入区滚动条。中文无空格长串在 421pt 面板下完整折行；面板缩至 389pt、357pt 和 325pt 时，高度依次按内容增至 572pt 和 589pt 上限，继续追加内容后输入区保持 589pt 并提供内部滚动条。最后清空草稿，窗口立即恢复 470pt，面板宽度恢复原来的约 421pt，暂存项目仍为 9 项。测试通过辅助功能驱动真实安装 App，没有按回车，未写入用户内容；输入法组合态、物理键盘连续输入手感和滚轮操作未覆盖。未生成 DMG，未更新 appcast、网站或 Sparkle。
- **2026-09-04 · v0.8.0 鼠标跟屏与输入换行版本发布**：版本升至 0.8.0（build 14），发布准备提交为 `4320ab4`。全新 DerivedData 的 146 项测试全部通过。`dist/ForNow-0.8.0.dmg` 为 4,562,417 bytes，SHA-256 `3290135092972b8975c83eec3fcd53398d3c45b14177253e4fedbe4e356cbe24`；完成 Developer ID 签名、Apple 公证 `Accepted`（提交号 `d9037ec5-2db9-4bfa-b440-e9e2739acf16`，公证日志 `issues: null`）与票据装订，DMG 和只读挂载 App 均由 Gatekeeper 识别为 `Notarized Developer ID`。包内 App 为 0.8.0（14）、Universal `x86_64 arm64`、最低 macOS 14.0、无 `get-task-allow`；主程序 SHA-256 `56fe02c7610db481399466a950922980360296ad02978c1754ba5f1674437630`，arm64 CDHash `6add394de7af8f97564f8aa6af198b3825350300`。Sparkle appcast SHA-256 为 `46390eb8968995f419bc40f2667ee24870031910a6c1918cb261dcf2eccf7fbf`，首项为 build 14，并生成从 build 13/12/11/10/9 升级的 5 个签名 delta；相邻 `ForNow14-13.delta` 为 235,998 bytes，SHA-256 `18f55689a36ea7c6ef11e209f1ebe90ad570b1f949d71418403b0c7bda221623`。实际把该 delta 应用于已公证正式 0.7.9 后，重建 App 通过深度验签，主程序哈希和 CDHash 均与完整 0.8.0 包一致。Cloudflare Pages 生产部署 `743c84e5-6f58-443d-8b84-edc291f9c0ff` 已上线；首页、独立说明、appcast、两份 0.8.0 DMG 及 feed 引用的全部 delta 均返回 200，本地 `dist`、站点源和远程 DMG 哈希一致，旧 0.7.9 DMG 哈希保持不变，不存在的版本返回 404。appcast 为 5 分钟缓存，版本化说明、DMG 和 delta 为一年 immutable；生产首页与部署专属地址哈希一致，1440×900 和 390×844 浏览器检查无横向溢出、破图或旧下载链接。站点发布前快照保存在 `/Users/liuyuhang/Library/Application Support/ForNow Release Backups/fornow_site-before-0.8.0-20260904-084638`。生成 feed 时历史归档中已有同名文件，出现非破坏性的 `Code=516` 警告；新 feed、签名、长度、delta 与线上哈希均已逐项验证。为执行 N−1 更新检查，本机先恢复了正式 0.7.9（13），但 Mac 锁屏使生产 Sparkle 更新窗口无法操作，因此该 UI 链路仍未验收。随后从公证 DMG 手动安装正式 0.8.0：`/Applications/ForNow.app` 为 0.8.0（14）、Universal，主程序哈希与包内一致，Gatekeeper 为 `Notarized Developer ID`，进程从明确安装路径运行；原正式 0.7.9 保存在 `/Users/liuyuhang/Library/Application Support/ForNow Installer Backups/ForNow-formal-0.7.9-before-0.8.0-manual-20260904-085542.app`。显示器选择、约 421pt 面板宽度、9 个活动项目和 27 个回收站项目均保留。源码未推送。
