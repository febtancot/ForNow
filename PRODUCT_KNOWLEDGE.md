...
- **多屏窗口模型**：`DisplayAttachmentSelection` 使用自动默认、指定屏幕和全部关闭三态；首次使用及「恢复默认显示」优先选择带刘海的屏幕，其次主屏。`NotchController` 分别维护可见胶囊屏、隐藏刘海触发屏和临时面板屏，共享同一份暂存、草稿、录音和播放状态。所有已选屏幕可同时显示收起态小药丸，但任一时刻只允许一块屏幕展开；点击另一块屏幕会把面板切换过去。全部关闭时，带刘海屏保留覆盖刘海及其下方 26pt 的透明窗口，只渲染命中层，不实例化 mic、托盘或 DayDrop 胶囊内容，并关闭窗口阴影；点击或拖入仍可展开，收起后恢复透明热区。无刘海屏不保留不可见窗口；菜单栏、全局快捷键或外部文件导入会按需创建临时面板，收起后立即移除。无刘海可见胶囊使用 `visibleFrame.maxY` 作为吸附线并保留 2pt 间距；刘海屏可见胶囊仍在刘海下方按原逻辑底部对齐。屏幕参数变化后立即重建或重定位窗口。
