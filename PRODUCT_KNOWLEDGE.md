...
- 当前 `main` 已实现尚未发布的快捷键鼠标跟屏：快捷键展开时严格使用鼠标所在屏幕，未启用胶囊的屏幕使用临时窗口，收起后清理；目标屏被全屏策略屏蔽时不跨屏回退。完整 142 项单元测试通过，本地测试版已安装；当前自动化按键通道未触发 Carbon 全局热键，实际双屏快捷键落点仍待人工交互验收。
- 当前 `main` 已实现尚未发布的快速输入自动换行修复：输入区使用实际布局宽度驱动 TextKit 测高，修复可调面板和清空按钮显隐造成的临界行裁切，并补上连续输入节流的尾沿测量。完整 146 项单元测试通过，本地测试版已安装；已在实际 App 中验证中英文无空格长串、面板宽度变化、8 行封顶与内部滚动。输入法组合态和物理键盘连续输入手感仍需人工使用确认。
