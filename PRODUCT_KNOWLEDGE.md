...
- 当前 `main` 已允许关闭所有视觉胶囊，同时在刘海屏保留透明点击和拖入触发区。三态设置、旧偏好迁移、历史断开 ID、热插拔保持全关、隐藏触发屏筛选和刘海最小热区回退已有纯逻辑测试；完整 139 项单元测试通过。本机 `/Applications/ForNow.app` 已替换为提交 `abdf1d7` 的同版本号 0.7.8（12）Universal 本地构建，并在两块屏幕都关闭胶囊后完成刘海点击、重复收起/展开和重启后再次点击验证；WindowServer 只保留内建刘海屏透明窗口，LG 外接屏无常驻 ForNow 窗口。Finder 拖入、全屏、热插拔、像素级无残留和 VoiceOver 仍需人工验收；本地构建未公证，尚未打包或发布新版本。
