...
- 当前 `main` 已允许在设置中关闭所有显示器上的收起态胶囊，同时保持首次使用默认开启一块屏幕。三态设置、旧偏好迁移、历史断开 ID、热插拔保持全关和临时面板落屏已有纯逻辑测试；完整 136 项单元测试通过。设置实际点击、全关后的胶囊消失、菜单栏/快捷键临时展开与跨重启行为仍需本机交互验收；尚未打包或发布新版本。
