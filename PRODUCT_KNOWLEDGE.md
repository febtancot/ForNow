...
- **2026-08-26 · Chrome/YouTube 全屏逐屏隐藏**：新增 `FullScreenVisibilityMonitor` 与可单测的 `FullScreenWindowDetector`。后台每 350ms 读取 onscreen WindowServer 快照，按各屏 Quartz bounds 识别覆盖率至少 99.5% 的其他进程 layer-0 窗口；默认只隐藏被覆盖屏幕的药丸，退出全屏自动恢复，显式开启全屏设置时跳过。新增 5 项检测测试，完整 117 项单测通过。三屏实测中，Studio Display 的 YouTube `2560×1440` 全屏出现后，该屏药丸从 onscreen 列表消失，内建屏与 Mi Monitor 药丸保留；退出 YouTube 全屏后 Studio 药丸自动恢复。此时验证的是 build 目录 Debug 构建，尚未替换 `/Applications` 安装副本。
