...
struct NotchRootView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            if controller.isOpen(on: displayID) {
                OpenPanelView()
                    .transition(.opacity)
            } else {
                ClosedPillView(displayID: displayID)
                    .transition(.opacity)
            }
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top)
        .animation(settings.animations ? .easeInOut(duration: 0.18) : nil,
                   value: controller.isOpen(on: displayID))
    }
}
...
struct ClosedPillView: View {
    ...
    var body: some View {
        ...
        .onHover { hovering = $0 }
        .onTapGesture { controller.open(on: displayID) }
        .onDrop(of: supportedDropTypes,
                isTargeted: Binding(get: { false },
                                    set: { if $0 { controller.openForDrag(on: displayID) } })) { providers in
            controller.openForDrag(on: displayID)
...
struct ClosedCapsuleBar: View {
    ...
    private var micSegment: some View {
        Button { controller.toggleRecording(on: displayID) } label: {
            HStack(spacing: 4) {
    ...
    private var traySegment: some View {
        Button { controller.open(on: displayID) } label: {
            HStack(spacing: 6) {
